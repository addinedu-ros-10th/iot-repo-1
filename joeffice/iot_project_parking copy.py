...
def fetch_parking_manage_rows():
    """
    마지막 주차 세션 기준으로 관리용 데이터 조회.
    - is_parked=1 이면 현재 진행 중 세션: last_in_time ~ NOW()
    - is_parked=0 이면 가장 최근 세션: last_in_time ~ last_out_time
    반환: list of dict
    """
...
class ManualRegisterDialog(QDialog):
    ...
    def on_save(self):
        ...
        klass = self.classCombo.currentText().strip()
        if not name:
            QMessageBox.information(self, "안내", "이름을 입력하세요.")
            return
        if not company:
            QMessageBox.information(self, "안내", "소속회사를 입력하세요.")
            return

        try:
            insert_parking_row(name, company, valid, klass)
        except Exception as e:
            QMessageBox.critical(self, "DB 오류", f"저장 실패: {e}")
            return
...
class MainWindow(BaseClass, UiClass):
    def __init__(self):
        ...
        if self.parkingCountLabel is None:
            raise RuntimeError("ui에 QLabel 'label_2'이 없습니다. (현재 주차 대수 표시)")

        self.tableWidget = self.findChild(QTableWidget, "tableWidget")
        if self.tableWidget is None:
            raise RuntimeError("ui에 QTableWidget 'tableWidget'이 없습니다. (현재 주차 차량 목록)")
        ...
        try:
            ...
            ensure_parking_schema()  # 컬럼/인덱스 보강
        except Exception as e:
    ...
    def on_plate_detected(self, plate: str):
        ...
        # 등록 차량이면 자동 OPEN + IN/OUT 토글 + UI 갱신
    ...
    def on_manage_clicked(self):
        """manageButton 클릭 시 tableWidget_2 토글(show/hide) + 보일 때 데이터 로드"""
        if self.tableWidget2.isVisible():
            self.tableWidget2.setVisible(False)
            return

        # 숨겨져 있던 상태 → 보이게 하면서 최신 데이터 로드
        try:
    ...
    def refresh_parking_summary(self):
        ...
        # tableWidget: 현재 주차 차량 목록
        try:
            rows = fetch_current_parked_rows()  # list[tuple]
