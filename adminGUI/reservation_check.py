...
class ReservationCheckWindow(QMainWindow):
    ...
    def handle_expired_reservations(self):
        ...
        try:
            ...
            # 종료 시간이 현재 UTC 시간보다 이전이고, 상태가 'BOOKED'인 예약을 찾음
            cursor.execute("""
    ...
    def find_auth_code(self):
        """직원 코드와 이름으로 예약 인증번호를 조회하는 함수 (UTC 기준)"""
        ...
        try:
            cursor = self.db_conn.cursor()
            # 현재 시간을 UTC 기준으로 가져와 쿼리에 사용
            now_utc = datetime.utcnow()
            
            print(f"디버깅 - 현재 시간 (UTC): {now_utc}")
            print(f"디버깅 - 입력된 UID: {uid}, 이름: {user_name}")

            # uid와 name으로 유효한 BOOKED 또는 CHECKED_IN 상태의 모든 예약을 찾음 (UTC 시간 기준)
            cursor.execute("""
            ...
            """, (uid, user_name, now_utc, now_utc, now_utc))

            results = cursor.fetchall()
            print(f"디버깅 - 쿼리 결과 (UTC): {results}")
    ...
    def create_dynamic_widgets(self, reservations):
        """ 여러 예약 정보를 동적으로 생성하여 표시 (UTC -> KST 변환) """
        ...
        for auth_code, room_name, start_time_utc, end_time_utc in reservations:
            # DB에서 가져온 UTC 시간을 KST로 변환하여 표시
            start_time_kst = start_time_utc + kst_offset
            end_time_kst = end_time_utc + kst_offset

            reservation_info_label = QLabel(
                f"<b>회의실:</b> {room_name}<br>"
                f"<b>시작:</b> {start_time_kst.strftime('%Y-%m-%d %H:%M:%S')}<br>"
                f"<b>종료:</b> {end_time_kst.strftime('%Y-%m-%d %H:%M:%S')}<br>"
